...
def check_info():
    ...
    for x in range(6):
        Categories.objects.create(websites=website, title=f'Category {x+1}', position=x)
...
def images():
    ...
    Banners.objects.create(websites=website, images='images/banner-1.jpg', position=1)

    Banners.objects.create(websites=website, images='images/banner-2.jpg', position=2)
...
def products():
    ...
    for x in range(4):
        categories.append(Categories.objects.create(websites=website, title=f"Category {x+1}", position=x))

    for x in range(9):

        Products.objects.create(websites=website, categories=categories[0],
                                title=f'{categories[0].title} Product {x+1}', price=x+1, position=position)
        position += 1

        Products.objects.create(websites=website, categories=categories[0],
                                title=f'{categories[0].title} Promotional {x+1}', price=x+1,
                                promotional_price=float(f'{x}.{x}'), position=position)
        position += 1

        Products.objects.create(websites=website, categories=categories[0],
                                title=f'{categories[0].title} Price type 3 {x+1}', price_type='3', position=position)
        position += 1

        Products.objects.create(websites=website, categories=categories[1], show_on_home=False,
                                title=f'{categories[1].title} Product {x+1}', price=x+1, position=position)
        position += 1
        ...
        Products.objects.create(websites=website, categories=categories[1], show_on_home=False,
                                title=f'{categories[1].title} Price type 3 {x+1}', price_type='3', position=position)
        position += 1
